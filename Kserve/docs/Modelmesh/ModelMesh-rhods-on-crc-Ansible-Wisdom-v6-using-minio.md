# Model Mesh with RHODS on CRC using Minio for Ansible Wisdom Model on Watson Runtime
...
export wisdom_img_tag=wisdom-v6  
...
oc create -f ${COMMON_MANIFESTS_HOME}/subs-rhods-operator-${rhods_version}.yaml -n ${RHODS_OP_NS}
...
**Model Upgrade**
- From wisdom v1 to wisdom v6

~~~
oc delete pod minio --force -n ${MINIO_NS}
...
cd ans-wis-model/clientcalls_v2
chmod 777 grpcurl.sh

sed 's/gpu-version-inference-service-v05/syntax-izumo-en-custom-v2/g' -i ./grpcurl.sh
